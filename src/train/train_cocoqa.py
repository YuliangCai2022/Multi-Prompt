...
class COCOQATrainer(TaskTrainer):
    ...
    def train_step(self, model, batch: Dict, optimizer=None, scheduler=None, ewc=None, replay=None):
        ...
        div_output = None
        # for prompt not always use the same key
        reduce_sim = 0
        reduce_sim_image = 0
        if self.args.prompt == 1:
            reduce_sim = output[2]
            reduce_sim_image = output[3]
        ...
        if self.args.prompt == 1:
            if reduce_sim is None:
                reduce_sim = 0
            if reduce_sim_image is None:
                reduce_sim_image = 0
            loss = loss - 0.1 * reduce_sim - 0.1 * reduce_sim_image
    ...
    def train(self, model, replay_memory=None, ewc=None) -> Tuple[float, Dict]:
        ...
        if self.args.adapter == 1:
            model.set_active_adapters("cocoqa")
        ...
        # prompt frequency accumulation varialble
        prompt_freq = [0] * 100
        ...
        for epoch in range(self.num_epochs):
            ...
            eval_score = self.eval(model,prompt_freq)
            logger.info("Evaluation after epoch {}: {:.2f}".format(epoch+1, eval_score))
            wandb_logger.log({'cocoqa': {'val_score': eval_score}})
            
            if epoch == self.num_epochs - 1:
                wandb_logger.log({'cocoqa':{'val_prompt_freq':prompt_freq}})
                logger.info("prompt freq is " + str(prompt_freq))
        ...
        # log the prompt utilization distribution of whole test
        

        return best_score, best_model

    def eval(self, model, prompt_freq) -> float:
        ...
        for step, batch in enumerate(tqdm(self.cocoqa_val_dataloader, desc='Evaluating on COCOQA val set')):
            ...
            if len(output) > 4 and not self.args.dytox:
    ...
    def eval_forgetting(self, model, model_path: str) -> float:
        ...
        score = self.eval(model, prompt_freq)

        logger.info("the prompt freq is " + str(prompt_freq))
