...
class ViltEncoderWrapper(EncoderWrapper):
    ...
    def forward(self, embedding_output=None, attention_mask_output=None,combine_prompt=None,**encodings: Dict) -> torch.FloatTensor:
        '''
        ...
        output = self.vilt(**encodings,embedding_output=embedding_output, attention_mask_output=attention_mask_output, combine_prompt=combine_prompt)
...
class ViltContinualLearner(ContinualLearner):
    ...
    def __init__(self, 
        ...
        if self.args.prompt == 1:
            self.prompt = Prompt(length=5, embed_dim=encoder_dim, embedding_key='mean', prompt_init='uniform',
                    prompt_pool=True, prompt_key=True, pool_size=100, top_k=5, batchwise_prompt=True,
    ...
    def forward_single_image(self, task_key: str, images: List, texts: List[str]) -> (torch.FloatTensor, torch.FloatTensor):
        ...
        encodings = self.vilt_encoder.process_inputs(images, texts).to(self.device)
        #encodings['pixel_values'] = self.prompt(encodings['pixel_values'], prompt_mask=None, cls_features=None)
        ...
        text_embedding, image_embedding, attention_mask, image_mask = self.vilt_encoder.vilt.embeddings(encodings['input_ids'],
            encodings['attention_mask'],encodings['token_type_ids'],encodings['pixel_values'],encodings['pixel_mask'],None,None,image_token_type_idx=None)

        logger.info("input id shape is " + str(encodings['input_ids'].shape))
        logger.info("text_embedding shape is " + str(text_embedding.shape))
        logger.info("pixel_values shape is " + str(encodings['pixel_values'].shape))
        logger.info("image_embedding shape is " + str(image_embedding.shape))
        #logger.info("attention_mask size is " + str(encodings['attention_mask'].shape))
       
        original_embedding = torch.cat([text_embedding,image_embedding], dim=1)

        # 25 is the current lenght of the prompt
        reduce_sim = None
        reduce_sim_img = None
        combine_prompt = None
        prompt_freq = None
        ...
        encoder_output = self.vilt_encoder(**encodings,embedding_output=embeddings, attention_mask_output=attention_mask, combine_prompt = combine_prompt)
        ...
        return encoder_output, output_logits, reduce_sim, reduce_sim_img, prompt_freq
    ...
    def forward_multi_images(self, task_key: str, images: List[List], texts: List[str], num_images=2) -> (torch.FloatTensor, torch.FloatTensor):
        ...
        for i in range(num_images):
            ...
            # for prompt
            encodings['attention_mask'] = encodings['attention_mask'][:,:encodings['input_ids'].shape[1]]
            text_embedding, image_embedding, attention_mask, image_mask = self.vilt_encoder.vilt.embeddings(encodings['input_ids'],
            encodings['attention_mask'],encodings['token_type_ids'],encodings['pixel_values'],encodings['pixel_mask'],None,None,image_token_type_idx=encodings['image_token_type_idx'])
            ...
            if self.args.prompt == 1:
                ...
                attention_mask = torch.cat([torch.ones(attention_mask.shape[0],25).to(self.device),attention_mask],dim=1)
                text_embedding = self.prompt(text_embedding, prompt_mask=None, cls_features = None, t_or_i=0)
                if prompt_freq is None:
                    prompt_freq = text_embedding['prompt_idx'][0]
                else:
                    prompt_freq = torch.cat([prompt_freq,text_embedding['prompt_idx'][0]],dim=0)
                reduce_sim = text_embedding['reduce_sim']
                text_embedding = text_embedding['prompted_embedding']
            ...
            pooled_out = self.vilt_encoder(**encodings,embedding_output=embeddings, attention_mask_output=attention_mask,combine_prompt=combine_prompt)
        ...
        return pooled_output, output_logits, reduce_sim, reduce_sim_img, prompt_freq
    ...
    def get_backbone_transformer(self):
        return self.vilt_encoder.vilt
    ...
    def get_total_encoder_params(self):
        return sum([p.numel() for p in self.vilt_encoder.parameters()])

    def get_total_trainable_params(self):
        return sum([p.numel() for p in self.parameters() if p.requires_grad == True])

    def get_total_trainable_encoder_params(self):
        return sum([p.numel() for p in self.vilt_encoder.parameters() if p.requires_grad == True])
...
def create_vilt_continual_learner_model(args,
