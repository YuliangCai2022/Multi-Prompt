...
import transformers
#from transformers.adapters import AdapterConfig
#from adapters import ADAPTER_MAP, SUPPORTED_ADAPTER_METHODS, create_adapter_handler, AdapterHandler
...
def main():
    ...
    parser.add_argument("--adapter_config_file",
                        help="File containing Adapter configuration")
    ...
    parser.add_argument("--prompt", type=int, default = 0,
                        help="whether to use prompt learning method")
    ...
    model = create_vilt_continual_learner_model(args=args,model_name_or_path=args.pretrained_model_name,
    ...
    if args.freeze_num == 0:
        pass 
    elif args.freeze_num == -1:
        logger.info("freeze all layers")
        model.vilt_encoder.freeze_all_weights()
    else:
        model.vilt_encoder.freeze_bottom_k_layers(args.freeze_num)
    ...
    adapter_config = None
    if args.adapter == 1:
        adapter_config = yaml.safe_load(open(args.adapter_config_file))
        adapter_handler = create_adapter_handler(adapter_config=adapter_config, args=args, device=device)
        adapter_handler.add_adapters_to_model(model, args.ordered_cl_tasks)
        ...
        for task_num, task_key in enumerate(args.ordered_cl_tasks):
            ...
            if args.task_attention != 0:
                ...
                if args.parallel != 0:
                    ...
                    for key in model.task_tokens:
                        if sub_task_num != num_task :
                            # try to not to freeze the other task tokens but smaller the learning rate of replay
                            ############### modification!!!!!!!!!!!!!!!!!!!!!!!!!!!################
            ...
            if os.path.isfile(os.path.join(task_output_dir, 'model')):
                ...
                if args.adapter == 1:
                    ...
                    trainable_adapter_params = adapter_handler.get_trainable_adapter_params(model)
                    total_encoder_params = model.get_total_encoder_params() - adapter_handler.get_total_adapter_params(model)
                    logger.info('Trainable Adapter Parameters: {:.2f}M ({:.2f}% of full encoder)'.format(trainable_adapter_params*10**-6, (trainable_adapter_params/total_encoder_params*100)))
            ...
            if args.cl_algorithm == 'adapter':
                adapter_handler.save_adapter(model, task_key, experiment_output_dir=os.path.join(output_dir, 'checkpoints'))
